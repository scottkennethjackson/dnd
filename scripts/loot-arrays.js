const cursed = [
    ['Armor of Vulnerability', 'magic-items/5376-armor-of-vulnerability'],
    ['Berserker Axe', 'magic-items/5382-berserker-axe'],
    ['Demon Armor', 'magic-items/4619-demon-armor'],
    ['Shield of Missile Attraction', 'magic-items/4754-shield-of-missile-attraction'],
    //['Sword of Vengence', 'magic-items/5505-sword-of-vengeance']
];

const gemstone = [
    ['Gemstone, Alexandrite (500 GP)'],
    ['Gemstone, Amber (100 GP)'],
    ['Gemstone, Amethyst (100 GP)'],
    ['Gemstone, Aquamarine (500 GP)'],
    ['Gemstone, Azurite (10 GP)'],
    ['Gemstone, Banded Agate (10 GP)'],
    ['Gemstone, Black Opal (1000 GP)'],
    ['Gemstone, Black Pearl (500 GP)'],
    ['Gemstone, Black Sapphire (5000 GP)'],
    ['Gemstone, Bloodstone (50 GP)'],
    ['Gemstone, Blue Quartz (10 GP)'],
    ['Gemstone, Blue Sapphire (1000 GP)'],
    ['Gemstone, Blue Spinel (500 GP)'],
    ['Gemstone, Carnelian (50 GP)'],
    ['Gemstone, Chalcedony (50 GP)'],
    ['Gemstone, Chrysoberyl (50 GP)'],
    ['Gemstone, Citrine (50 GP)'],
    ['Gemstone, Coral (100 GP)'],
    ['Gemstone, Diamond (5000 GP)'],
    ['Gemstone, Emerald (1000 GP)'],
    ['Gemstone, Eye Agate (10 GP)'],
    ['Gemstone, Fire Opal (1000 GP)'],
    ['Gemstone, Garnet (100 GP)'],
    ['Gemstone, Hematite (10 GP)'],
    ['Gemstone, Jacinth (5000 GP)'],
    ['Gemstone, Jade (100 GP)'],
    ['Gemstone, Jasper (50 GP)'],
    ['Gemstone, Jet (100 GP)'],
    ['Gemstone, Lapis Lazuli (10 GP)'],
    ['Gemstone, Malachite (10 GP)'],
    ['Gemstone, Moonstone (50 GP)'],
    ['Gemstone, Moss Agate (10 GP)'],
    ['Gemstone, Obsidian (10 GP)'],
    ['Gemstone, Onyx (50 GP)'],
    ['Gemstone, Opal (1000 GP)'],
    ['Gemstone, Pearl (100 GP)'],
    ['Gemstone, Peridot (500 GP)'],
    ['Gemstone, Quartz (50 GP)'],
    ['Gemstone, Rhodochrosite (10 GP)'],
    ['Gemstone, Ruby (5000 GP)'],
    ['Gemstone, Sardonyx (50 GP)'],
    ['Gemstone, Spinel (100 GP)'],
    ['Gemstone, Star Rose Quartz (50 GP)'],
    ['Gemstone, Star Ruby (1000 GP)'],
    ['Gemstone, Star Sapphire (1000 GP)'],
    ['Gemstone, Tiger Eye (10 GP)'],
    ['Gemstone, Topaz (500 GP)'],
    ['Gemstone, Tourmaline (100 GP)'],
    ['Gemstone, Turquoise (10 GP)'],
    ['Gemstone, Yellow Sapphire (1000 GP)'],
    ['Gemstone, Zircon (50 GP)']
];

const treasure = [
    ['Silver Ewer (25 GP)'],
    ['Carved Bone Statuette (25 GP)'],
    ['Small Gold Bracelet (25 GP)'],
    ['Cloth-of-Gold Vestments (25 GP)'],
    ['Black Velvet Mask Stitched with Silver Thread (25 GP)'],
    ['Copper Chalice with Silver Filigree (25 GP)'],
    ['Pair of Engraved Bone Dice (25 GP)'],
    ['Small Mirror Set in a Painted Wooden Frame (25 GP)'],
    ['Embroidered Silk Handkerchief (25 GP)'],
    ['Gold Locket with a Painted Portrait Inside (25 GP)'],
    ['Gold Ring Set with Bloodstones (250 GP)'],
    ['Carved Ivory Statuette (250 GP)'],
    ['Large Gold Bracelet (250 GP)'],
    ['Silver Necklace with a Gemstone Pendant (250 GP)'],
    ['Bronze Crown (250 GP)'],
    ['Silk Robe with Gold Embroidery (250 GP)'],
    ['Large Well-Made Tapestry (250 GP)'],
    ['Brass Mug with Jade Inlay (250 GP)'],
    ['Box of Turquoise Animal Figurines (250 GP)'],
    ['Gold Bird Cage with Electrum Filigree (250 GP)'],
    ['Silver Chalice Set with Moonstones (750 GP)'],
    ['Silver-Plated Steel Longsword with Jet Set in Hilt (750 GP)'],
    ['Carved Harp of Exotic Wood with Ivory Inlay and Zircon Gems (750 GP)'],
    ['Small Gold Idol (750 GP)'],
    ['Gold Dragon Comb Set with Red Garnets as Eyes (750 GP)'],
    ['Bottle Stopper Cork Embossed with Gold Leaf and Set with Amethysts (750 GP)'],
    ['Ceremonial Electrum Dagger with a Black Pearl in the Pommel (750 GP)'],
    ['Silver and Gold Brooch (750 GP)'],
    ['Obsidian Statuette with Gold Fittings and Inlay (750 GP)'],
    ['Painted Gold War Mask (750 GP)'],
    ['Fine Gold Chain Set with a Fire Opal (2500 GP)'],
    ['Old Masterpiece Painting (2500 GP)'],
    ['Embroidered Silk and Velvet Mantle Set with Numerous Moonstones (2500 GP)'],
    ['Platinum Bracelet Set with a Sapphire (2500 GP)'],
    ['Embroidered Glove Set with Jewel Chips (2500 GP)'],
    ['Jeweled Anklet (2500 GP)'],
    ['Gold Music Box (2500 GP)'],
    ['Gold Circlet Set with Four Aquamarines (2500 GP)'],
    ['Eye Patch with a Mock Eye Set in Blue Sapphire and Moonstone (2500 GP)'],
    ['A Necklace String of Small Pink Pearls (2500 GP)'],
    ['Jeweled Gold Crown (7500 GP)'],
    ['Jeweled Platinum Ring (7500 GP)'],
    ['Small Gold Statuette Set with Rubies (7500 GP)'],
    ['Gold Cup Set with Emeralds (7500 GP)'],
    ['Gold Jewelry Box with Platinum Filigree (7500 GP)'],
    [`Painted Gold Child's Sarcophagus (7500 GP)`],
    ['Jade Game Board with Solid Gold Playing Pieces (7500 GP)'],
    ['Bejeweled Ivory Drinking Horn with Gold Filigree (7500 GP)']
];

const consumable = [
    [`Alchemist's Fire, Flask`, 'equipment/19-alchemists-fire-flask'],
    [`Assassin's Blood, Ingested`, 'equipment/137-assassins-blood-ingested'],
    ['Bead of Force', 'magic-items/4583-bead-of-force'],
    //['Bead of Nourishment', 'magic-items/27022-bead-of-nourishment'],
    //['Bead of Refreshment', 'magic-items/27023-bead-of-refreshment'],
    ['Burnt Othur Fumes, Inhaled', 'equipment/138-burnt-othur-fumes-inhaled'],
    ['Candle of Invocation', 'magic-items/4598-candle-of-invocation'],
    ['Chime of Opening', 'magic-items/4602-chime-of-opening'],
    ['Crawler Mucus, Contact', 'equipment/139-crawler-mucus-contact'],
    ['Deck of Illusions', 'magic-items/4616-deck-of-illusions'],
    ['Deck of Many Things', 'magic-items/4617-deck-of-many-things'],
    ['Drow Poison, Injury', 'equipment/140-drow-poison-injury'],
    ['Dust of Disappearance', 'magic-items/4623-dust-of-disappearance'],
    ['Dust of Dryness', 'magic-items/4624-dust-of-dryness'],
    ['Dust of Sneezing and Choking', 'magic-items/4625-dust-of-sneezing-and-choking'],
    ['Efreeti Bottle', 'magic-items/4629-efreeti-bottle'],
    ['Elemental Gem, Blue Sapphire', 'magic-items/5374-elemental-gem'],
    ['Elemental Gem, Emerald', 'magic-items/5374-elemental-gem'],
    ['Elemental Gem, Red Corundum', 'magic-items/5374-elemental-gem'],
    ['Elemental Gem, Yellow Diamond', 'magic-items/5374-elemental-gem'],
    //['Elixir of Health', 'magic-items/5351-elixir-of-health'],
    ['Essence of Ether, Inhaled', 'equipment/141-essence-of-ether-inhaled'],
    ['Feather Token, Anchor', 'magic-items/4636-feather-token-anchor'],
    ['Feather Token, Bird', 'magic-items/4890-feather-token-bird'],
    ['Feather Token, Fan', 'magic-items/4891-feather-token-fan'],
    ['Feather Token, Swan Boat', 'magic-items/4892-feather-token-swan-boat'],
    ['Feather Token, Tree', 'magic-items/4893-feather-token-tree'],
    ['Feather Token, Whip', 'magic-items/4894-feather-token-whip'],
    ['Malice, Inhaled', 'equipment/142-malice-inhaled'],
    ['Manual of Clay Golems', 'magic-items/5416-manual-of-golems'],
    ['Manual of Flesh Golems', 'magic-items/5416-manual-of-golems'],
    ['Manual of Iron Golems', 'magic-items/5416-manual-of-golems'],
    ['Manual of Stone Golems', 'magic-items/5416-manual-of-golems'],
    ['Marvelous Pigments', 'magic-items/4678-marvelous-pigments'],
    ['Midnight Tears, Ingested', 'equipment/143-midnight-tears-ingested'],
    //['Nolzur\'s Marvelous Pigments', 'magic-items/4678-marvelous-pigments'],
    ['Oil of Etherealness', 'magic-items/4687-oil-of-etherealness'],
    ['Oil of Sharpness', 'magic-items/4688-oil-of-sharpness'],
    ['Oil of Slipperiness', 'magic-items/4689-oil-of-slipperiness'],
    ['Oil of Taggit, Contact', 'equipment/144-oil-of-taggit-contact'],
    ['Pale Tincture, Ingested', 'equipment/145-pale-tincture-ingested'],
    //['Perfume of Bewitching', 'magic-items/27079-perfume-of-bewitching'],
    ['Philter of Love', 'magic-items/4695-philter-of-love'],
    ['Poison, Basic', 'equipment/68-poison-basic-vial'],
    ['Potion of Acid Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Animal Friendship', 'magic-items/4700-potion-of-animal-friendship'],
    ['Potion of Clairvoyance', 'magic-items/4701-potion-of-clairvoyance'],
    ['Potion of Climbing', 'magic-items/4702-potion-of-climbing'],
    ['Potion of Cloud Giant Strength', 'magic-items/5417-potion-of-giant-strength'],
    ['Potion of Cold Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Diminution', 'magic-items/4703-potion-of-diminution'],
    //['Potion of Fire Breath', 'magic-items/5357-potion-of-fire-breath'],
    ['Potion of Fire Giant Strength', 'magic-items/5417-potion-of-giant-strength'],
    ['Potion of Fire Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Flying', 'magic-items/4704-potion-of-flying'],
    ['Potion of Force Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Frost Giant Strength', 'magic-items/5417-potion-of-giant-strength'],
    ['Potion of Gaseous Form', 'magic-items/4705-potion-of-gaseous-form'],
    ['Potion of Greater Healing', 'magic-items/4708-potion-of-healing'],
    ['Potion of Growth', 'magic-items/4707-potion-of-growth'],
    ['Potion of Heroism', 'magic-items/4709-potion-of-heroism'],
    ['Potion of Hill Giant Strength', 'magic-items/5417-potion-of-giant-strength'],
    ['Potion of Invisibility', 'magic-items/4710-potion-of-invisibility'],
    //['Potion of Invulnerability', 'magic-items/5358-potion-of-invulnerability'],
    ['Potion of Lightning Resistance', 'magic-items/5419-potion-of-resistance'],
    //['Potion of Longevity', 'magic-items/5359-potion-of-longevity'],
    ['Potion of Mind Reading', 'magic-items/4711-potion-of-mind-reading'],
    ['Potion of Necrotic Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Poison', 'magic-items/4712-potion-of-poison'],
    ['Potion of Poison Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Psychic Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Radiant Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Speed', 'magic-items/4714-potion-of-speed'],
    ['Potion of Stone Giant Strength', 'magic-items/5417-potion-of-giant-strength'],
    ['Potion of Storm Giant Strength', 'magic-items/5417-potion-of-giant-strength'],
    ['Potion of Superior Healing', 'magic-items/4708-potion-of-healing'],
    ['Potion of Supreme Healing', 'magic-items/4708-potion-of-healing'],
    ['Potion of Thunder Resistance', 'magic-items/5419-potion-of-resistance'],
    ['Potion of Vitality', 'magic-items/5360-potion-of-vitality'],
    ['Potion of Water Breathing', 'magic-items/4715-potion-of-water-breathing'],
    ['Purple Worm Poison, Injury', 'equipment/146-purple-worm-poison-injury'],
    //['Quaal\'s Feather Token, Anchor', 'magic-items/13584-quaals-feather-token'],
    //['Quaal\'s Feather Token, Bird', 'magic-items/13584-quaals-feather-token'],
    //['Quaal\'s Feather Token, Fan', 'magic-items/13584-quaals-feather-token'],
    //['Quaal\'s Feather Token, Swan Boat', 'magic-items/13584-quaals-feather-token'],
    //['Quaal\'s Feather Token, Tree', 'magic-items/13584-quaals-feather-token'],
    //['Quaal\'s Feather Token, Whip', 'magic-items/13584-quaals-feather-token'],
    ['Restorative Ointment', 'magic-items/4716-restorative-ointment'],
    ['Ring of Three Wishes', 'magic-items/4735-ring-of-three-wishes'],
    ['Scarab of Protection', 'magic-items/4751-scarab-of-protection'],
    ['Serpent Venom, Injury', 'equipment/147-serpent-venom-injury'],
    ['Sovereign Glue', 'magic-items/4756-sovereign-glue'],
    ['Spell Scroll, Cantrip', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 1st Level', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 2nd Level', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 3rd Level', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 4th Level', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 5th Level', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 6th Level', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 7th Level', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 8th Level', 'magic-items/5418-spell-scroll'],
    ['Spell Scroll, 9th Level', 'magic-items/5418-spell-scroll'],
    ['Torpor, Ingested', 'equipment/148-torpor-ingested'],
    ['Truth Serum, Ingested', 'equipment/149-truth-serum-ingested'],
    ['Universal Solvent', 'magic-items/4785-universal-solvent'],
    ['Wyvern Poison, Injury', 'equipment/150-wyvern-poison-injury'],
];

const cantrips = [
    ['Acid Splash', 'spells/1989-acid-splash'],
    ['Chill Touch', 'spells/2026-chill-touch'],
    ['Dancing Lights', 'spells/2057-dancing-lights'],
    ['Fire Bolt', 'spells/2103-fire-bolt'],
    ['Light', 'spells/2166-light'],
    ['Mage Hand', 'spells/2173-mage-hand'],
    ['Mending', 'spells/2187-mending'],
    ['Message', 'spells/2188-message'],
    ['Minor Illusion', 'spells/2191-minor-illusion'],
    ['Poison Spray', 'spells/2208-poison-spray'],
    ['Prestidigitation', 'spells/2213-prestidigitation'],
    ['Ray of Frost', 'spells/2226-ray-of-frost'],
    ['Shocking Grasp', 'spells/2250-shocking-grasp'],
    ['True Strike', 'spells/2287-true-strike']
];

const spells1st = [
    ['Alarm', 'spells/1991-alarm'],
    ['Burning Hands', 'spells/2021-burning-hands'],
    ['Charm Person', 'spells/2025-charm-person'],
    ['Color Spray', 'spells/2031-color-spray'],
    ['Comprehend Languages', 'spells/2035-comprehend-languages'],
    ['Detect Magic', 'spells/2065-detect-magic'],
    ['Disguise Self', 'spells/2069-disguise-self'],
    ['Expedious Retreat', 'spells/2088-expeditious-retreat'],
    ['False Life', 'spells/2093-false-life'],
    ['Feather Fall', 'spells/2095-feather-fall'],
    ['Find Familiar', 'spells/2097-find-familiar'],
    ['Floating Disk', 'spells/2110-floating-disk'],
    ['Fog Cloud', 'spells/2112-fog-cloud'],
    ['Grease', 'spells/2127-grease'],
    ['Hideous Laughter', 'spells/2145-hideous-laughter'],
    ['Identify', 'spells/2152-identify'],
    ['Illusory Script', 'spells/2153-illusory-script'],
    ['Jump', 'spells/2161-jump'],
    ['Longstrider', 'spells/2171-longstrider'],
    ['Mage Armor', 'spells/2172-mage-armor'],
    ['Magic Missile', 'spells/2176-magic-missile'],
    ['Protection from Evil and Good', 'spells/2221-protection-from-evil-and-good'],
    ['Shield', 'spells/2247-shield'],
    ['Silent Image', 'spells/2252-silent-image'],
    ['Sleep', 'spells/2254-sleep'],
    ['Thunderwave', 'spells/2278-thunderwave'],
    ['Unseen Servant', 'spells/2288-unseen-servant']
];

const spells2nd = [
    ['Acid Arrow', 'spells/1988-acid-arrow'],
    ['Alter Self', 'spells/1992-alter-self'],
    ['Arcane Lock', 'spells/2003-arcane-lock'],
    [`Arcanist's Magic Aura`, 'spells/2005-arcanists-magic-aura'],
    ['Blindness/Deafness', 'spells/2018-blindness-deafness'],
    ['Blur', 'spells/2020-blur'],
    ['Continual Flame', 'spells/2048-continual-flame'],
    ['Darkness', 'spells/2058-darkness'],
    ['Darkvision', 'spells/2059-darkvision'],
    ['Detect Thoughts', 'spells/2067-detect-thoughts'],
    ['Enlarge/Reduce', 'spells/2084-enlarge-reduce'],
    ['Flaming Sphere', 'spells/2108-flaming-sphere'],
    ['Gentle Repose', 'spells/2121-gentle-repose'],
    ['Gust of Wind', 'spells/2134-gust-of-wind'],
    ['Hold Person', 'spells/2147-hold-person'],
    ['Invisibility', 'spells/2159-invisibility'],
    ['Knock', 'spells/2162-knock'],
    ['Levitate', 'spells/2165-levitate'],
    ['Locate Object', 'spells/2170-locate-object'],
    ['Magic Mouth', 'spells/2177-magic-mouth'],
    ['Magic Weapon', 'spells/2178-magic-weapon'],
    ['Mirror Image', 'spells/2193-mirror-image'],
    ['Misty Step', 'spells/2195-misty-step'],
    ['Ray of Enfeeblement', 'spells/2225-ray-of-enfeeblement'],
    ['Rope Trick', 'spells/2235-rope-trick'],
    ['Scorching Ray', 'spells/2238-scorching-ray'],
    ['See Invisibility', 'spells/2242-see-invisibility'],
    ['Shatter', 'spells/2246-shatter'],
    ['Spider Climb', 'spells/2261-spider-climb'],
    ['Suggestion', 'spells/2269-suggestion'],
    ['Web', 'spells/2299-web']
];

const spells3rd = [
    ['Animate Dead', 'splls/1996-animate-dead'],
    ['Bestow Curse', 'splls/2013-bestow-curse'],
    ['Blink', 'splls/2019-blink'],
    ['Clairvoyance', 'splls/2028-clairvoyance'],
    ['Counterspell', 'splls/2051-counterspell'],
    ['Dispell Magic', 'splls/2072-dispel-magic'],
    ['Fear', 'splls/2094-fear'],
    ['Fireball', 'splls/2102-fireball'],
    ['Fly', 'splls/2111-fly'],
    ['Gaseous Form', 'splls/2118-gaseous-form'],
    ['Glyph of Warding', 'splls/2125-glyph-of-warding'],
    ['Haste', 'splls/2138-haste'],
    ['Hypnotic Pattern', 'splls/2150-hypnotic-pattern'],
    ['Lightning Bolt', 'splls/2167-lightning-bolt'],
    ['Magic Circle', 'splls/2174-magic-circle'],
    ['Major Image', 'splls/2180-major-image'],
    ['Nondetection', 'splls/2199-nondetection'],
    ['Phantom Steed', 'splls/2203-phantom-steed'],
    ['Protection from Energy', 'splls/2220-protection-from-energy'],
    ['Remove Curse', 'splls/2229-remove-curse'],
    ['Sending', 'splls/2243-sending'],
    ['Sleet Storm', 'splls/2255-sleet-storm'],
    ['Slow', 'splls/2256-slow'],
    ['Stinking Cloud', 'splls/2265-stinking-cloud'],
    ['Tiny Hut', 'splls/2280-tiny-hut'],
    ['Tongues', 'splls/2281-tongues'],
    ['Vampiric Touch', 'splls/2289-vampiric-touch'],
    ['Water Breathing', 'splls/2297-water-breathing']
];

const spells4th = [
    ['Arcane Eye', 'spells/2001-arcane-eye'],
    ['Banishment', 'spells/2010-banishment'],
    ['Black Tentacles', 'spells/2014-black-tentacles'],
    ['Blight', 'spells/2017-blight'],
    ['Confusion', 'spells/2038-confusion'],
    ['Conjur Minor Elemental', 'spells/2043-conjure-minor-elementals'],
    ['Control Water', 'spells/2049-control-water'],
    ['Dimension Door', 'spells/2068-dimension-door'],
    ['Fabricate', 'spells/2090-fabricate'],
    ['Faithful Hound', 'spells/2092-faithful-hound'],
    ['Fire Shield', 'spells/2104-fire-shield'],
    ['Greater Invisibility', 'spells/2128-greater-invisibility'],
    ['Hallucinatory Terrain', 'spells/2136-hallucinatory-terrain'],
    ['Ice Storm', 'spells/2151-ice-storm'],
    ['Locate Creature', 'spells/2169-locate-creature'],
    ['Phantasmal Killer', 'spells/2202-phantasmal-killer'],
    ['Polymorph', 'spells/2209-polymorph'],
    ['Private Sanctum', 'spells/2216-private-sanctum'],
    ['Resilient Sphere', 'spells/2230-resilient-sphere'],
    ['Secret Chest', 'spells/2240-secret-chest'],
    ['Stone Shape', 'spells/2267-stone-shape'],
    ['Stoneskin', 'spells/2266-stoneskin'],
    ['Wall of Fire', 'spells/2291-wall-of-fire']
];

const spells5th = [
    ['Animate Objects', 'spells/1997-animate-objects'],
    ['Arcane Hand', 'spells/2002-arcane-hand'],
    ['Cloudkill', 'spells/2030-cloudkill'],
    ['Cone of Cold', 'spells/2037-cone-of-cold'],
    ['Conjur Elemental', 'spells/2041-conjure-elemental'],
    ['Contact Other Plane', 'spells/2045-contact-other-plane'],
    ['Creation', 'spells/2055-creation'],
    ['Dominate Person', 'spells/2078-dominate-person'],
    ['Dream', 'spells/2079-dream'],
    ['Geas', 'spells/2120-geas'],
    ['Hold Monster', 'spells/2146-hold-monster'],
    ['Legend Lore', 'spells/2163-legend-lore'],
    ['Mislead', 'spells/2194-mislead'],
    ['Modify Memory', 'spells/2196-modify-memory'],
    ['Passwell', 'spells/2200-passwall'],
    ['Planar Building', 'spells/2205-planar-binding'],
    ['Scrying', 'spells/2239-scrying'],
    ['Seeming', 'spells/2241-seeming'],
    ['Telekinesis', 'spells/2273-telekinesis'],
    ['Telepathic Bond', 'pells/2274-telepathic-bond'],
    ['Teleportation Circle', 'spells/2276-teleportation-circle'],
    ['Wall of Force', 'spells/2292-wall-of-force'],
    ['Wall of Stone', 'spells/2294-wall-of-stone']
];

const spells6th = [
    ['Chain Lightning', 'spells/2024-chain-lightning'],
    ['Circle of Death', 'spells/2027-circle-of-death'],
    ['Contingency', 'spells/2047-contingency'],
    ['Create Undead', 'spells/2054-create-undead'],
    ['Disintegrate', 'spells/2070-disintegrate'],
    ['Eyebite', 'spells/2089-eyebite'],
    ['Flesh to Stone', 'spells/2109-flesh-to-stone'],
    ['Freezing Sphere', 'spells/2117-freezing-sphere'],
    ['Globe of Invulnerability', 'spells/2124-globe-of-invulnerability'],
    ['Guards and Wands', 'spells/2131-guards-and-wards'],
    ['Instant Summons', 'spells/2158-instant-summons'],
    ['Magic Jar', 'spells/2175-magic-jar'],
    ['Mass Suggestion', 'spells/2184-mass-suggestion'],
    ['Move Earth', 'spells/2198-move-earth'],
    [`Otto's Irresistible Dance`, 'spells/2160-ottos-irresistible-dance'],
    ['Programmed Illusion', 'spells/2218-programmed-illusion'],
    ['Sunbeam', 'spells/2270-sunbeam'],
    ['True Seeing', 'spells/2286-true-seeing'],
    ['Wall of Ice', 'spells/2293-wall-of-ice']
];

const spells7th = [
    ['Delayed Blast Fireball', 'spells/2062-delayed-blast-fireball'],
    ['Etherealness', 'spells/2087-etherealness'],
    ['Finger of Death', 'spells/2101-finger-of-death'],
    ['Forcecage', 'spells/2114-forcecage'],
    ['Magnificent Mansion', 'spells/2179-magnificent-mansion'],
    ['Mirage Arcane', 'spells/2192-mirage-arcane'],
    [`Mordenkainen's Sword`, 'spells/2004-mordenkainens-sword'],
    ['Plane Shift', 'spells/2206-plane-shift'],
    ['Prismatic Spray', 'spells/2214-prismatic-spray'],
    ['Project Image', 'spells/2219-project-image'],
    ['Reverse Gravity', 'spells/2233-reverse-gravity'],
    ['Sequester', 'spells/2244-sequester'],
    ['Simulacrum', 'spells/2253-simulacrum'],
    ['Symbol', 'spells/2272-symbol'],
    ['Teleport', 'spells/2275-teleport']
];

const spells8th = [
    ['Antimagic Field', 'spells/1999-antimagic-field'],
    ['Antipathy/Sympathy', 'spells/2000-antipathy-sympathy'],
    ['Clone', 'spells/2029-clone'],
    ['Control Weather', 'spells/2050-control-weather'],
    ['Demiplane', 'spells/2063-demiplane'],
    ['Dominate Monster', 'spells/2077-dominate-monster'],
    ['Feeblemind', 'spells/2096-feeblemind'],
    ['Incendiary Cloud', 'spells/2155-incendiary-cloud'],
    ['Maze', 'spells/2185-maze'],
    ['Mind Blank', 'spells/2190-mind-blank'],
    ['Power Word Stun', 'spells/2211-power-word-stun'],
    ['Sunburst', 'spells/2271-sunburst']
];

const spells9th = [
    ['Astral Projection', 'spells/2006-astral-projection'],
    ['Foresight', 'spells/2115-foresight'],
    ['Gate', 'spells/2119-gate'],
    ['Imprisonment', 'spells/2154-imprisonment'],
    ['Meteor Swarm', 'spells/2189-meteor-swarm'],
    ['Power Word Kill', 'spells/2210-power-word-kill'],
    ['Prismatic Wall', 'spells/2215-prismatic-wall'],
    ['Shapechange', 'spells/2245-shapechange'],
    ['Time Stop', 'spells/2279-time-stop'],
    ['True Polymorph', 'spells/2284-true-polymorph'],
    ['Weird', 'spells/2300-weird'],
    ['Wish', 'spells/2303-wish']
];

const common = [
    ['Armor of Gleaming', 'magic-items/36571-armor-of-gleaming'],
    ['Boots of False Tracks', 'magic-items/27025-boots-of-false-tracks'],
    ['Candle of the Deep', 'magic-items/27026-candle-of-the-deep'],
    ['Cast-Off Armor', 'magic-items/36589-cast-off-armor'],
    ['Charlatan\'s Die', 'magic-items/27039-charlatans-die'],
    ['Cloak of Billowing', 'magic-items/27040-cloak-of-billowing'],
    ['Cloak of Many Fashions', 'magic-items/27041-cloak-of-many-fashions'],
    ['Clockwork Amulet', 'magic-items/27042-clockwork-amulet'],
    ['Clothes of Mending', 'magic-items/27043-clothes-of-mending'],
    ['Dark Shard Amulet', 'magic-items/27044-dark-shard-amulet'],
    ['Dread Helm', 'magic-items/27045-dread-helm'],
    ['Ear Horn of Hearing', 'magic-items/27046-ear-horn-of-hearing'],
    ['Enduring Spellbook', 'magic-items/27047-enduring-spellbook'],
    ['Ersatz Eye', 'magic-items/27048-ersatz-eye'],
    ['Hat of Vermin', 'magic-items/27049-hat-of-vermin'],
    ['Hat of Wizardry', 'magic-items/27050-hat-of-wizardry'],
    ['Heward\'s Handy Spice Pouch', 'magic-items/27063-hewards-handy-spice-pouch'],
    ['Horn of Silent Alarm', 'magic-items/27064-horn-of-silent-alarm'],
    ['Instrument of Illusions', 'magic-items/27065-instrument-of-illusions'],
    ['Instrument of Scribing', 'magic-items/27066-instrument-of-scribing'],
    ['Lock of Trickery', 'magic-items/27068-lock-of-trickery'],
    ['Moon-Touched Sword', 'magic-items/36822-moon-touched-sword'],
    ['Mystery Key', 'magic-items/27076-mystery-key'],
    ['Orb of Direction', 'magic-items/27077-orb-of-direction'],
    ['Orb of Time', 'magic-items/27078-orb-of-time'],
    ['Pipe of Smoke Monsters', 'magic-items/27082-pipe-of-smoke-monsters'],
    ['Pole of Angling', 'magic-items/27108-pole-of-angling'],
    ['Pole of Collapsing', 'magic-items/27112-pole-of-collapsing'],
    ['Pot of Awakening', 'magic-items/27113-pot-of-awakening'],
    ['Prosthetic Limb', 'magic-items/1000601-prosthetic-limb'],
    ['Rope of Mending', 'magic-items/27114-rope-of-mending'],
    ['Ruby of the War Mage', 'magic-items/27115-ruby-of-the-war-mage'],
    ['Shield of Expression', 'magic-items/27116-shield-of-expression'],
    ['Smoldering Armor', 'magic-items/36837-smoldering-armor'],
    ['Staff of Adornment', 'magic-items/27136-staff-of-adornment'],
    ['Staff of Birdcalls', 'magic-items/27137-staff-of-birdcalls'],
    ['Staff of Flowers', 'magic-items/27139-staff-of-flowers'],
    ['Talking Doll', 'magic-items/27140-talking-doll'],
    ['Tankard of Sobriety', 'magic-items/27142-tankard-of-sobriety'],
    ['Unbreakable Arrow', 'magic-items/36838-unbreakable-arrow'],
    ['Veteran\'s Cane', 'magic-items/27145-veterans-cane'],
    ['Walloping Ammunition', 'magic-items/36839-walloping-ammunition'],
    ['Wand of Conducting', 'magic-items/27146-wand-of-conducting'],
    ['Wand of Pyrotechnics', 'magic-items/27147-wand-of-pyrotechnics'],
    ['Wand of Scowls', 'magic-items/27148-wand-of-scowls'],
    ['Wand of Smiles', 'magic-items/27149-wand-of-smiles'],
];

const uncommon = [
    //['All-Purpose Tool, +1', 'magic-items/2400634-all-purpose-tool'],
    ['Adamantine Armor', 'magic-items/5370-adamantine-armor'],
    //['Alchemy Jug', 'magic-items/5346-alchemy-jug'],
    ['Ammunition, +1', 'magic-items/5407-ammunition-1'],
    ['Amulet of Proof Against Detection and Location', 'magic-items/4569-amulet-of-proof-against-detection-and-location'],
    //['Amulet of the Devout, +1', 'magic-items/2411830-amulet-of-the-devout'],
    //['Arcane Grimoire, +1', 'magic-items/2411893-arcane-grimoire'],
    ['Bag of Holding', 'magic-items/4581-bag-of-holding'],
    //['Bloodwell Vial, +1', 'magic-items/2412091-bloodwell-vial'],
    ['Boots of Elvenkind', 'magic-items/4587-boots-of-elvenkind'],
    ['Boots of Striding and Springing', 'magic-items/4590-boots-of-striding-and-springing'],
    ['Boots of the Winterlands', 'magic-items/4591-boots-of-the-winterlands'],
    ['Bracers of Archery', 'magic-items/4593-bracers-of-archery'],
    ['Brooch of Shielding', 'magic-items/4596-brooch-of-shielding'],
    ['Broom of Flying', 'magic-items/4597-broom-of-flying'],
    //['Cap of Water Breathing', 'magic-items/5345-cap-of-water-breathing'],
    ['Circlet of Blasting', 'magic-items/4603-circlet-of-blasting'],
    ['Cloak of Elvenkind', 'magic-items/4606-cloak-of-elvenkind'],
    ['Cloak of Protection', 'magic-items/4607-cloak-of-protection'],
    ['Cloak of the Manta Ray', 'magic-items/4609-cloak-of-the-manta-ray'],
    ['Decanter of Endless Water', 'magic-items/4615-decanter-of-endless-water'],
    //['Driftglobe', 'magic-items/5349-driftglobe'],
    ['Efficient Quiver', 'magic-items/4628-efficient-quiver'],
    ['Eversmoking Bottle', 'magic-items/4632-eversmoking-bottle'],
    ['Eyes of Charming', 'magic-items/4633-eyes-of-charming'],
    ['Eyes of Minute Seeing', 'magic-items/4634-eyes-of-minute-seeing'],
    ['Eyes of the Eagle', 'magic-items/4635-eyes-of-the-eagle'],
    //['Feywild Shard', 'magic-items/2412248-feywild-shard'],
    ['Figurine of Wondrous Power, Silver Raven', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Gauntlets of Ogre Power', 'magic-items/4641-gauntlets-of-ogre-power'],
    ['Gem of Brightness', 'magic-items/4642-gem-of-brightness'],
    ['Gloves of Missile Snaring', 'magic-items/4646-gloves-of-missile-snaring'],
    ['Gloves of Swimming and Climbing', 'magic-items/4647-gloves-of-swimming-and-climbing'],
    //['Gloves of Thievery', 'magic-items/5352-gloves-of-thievery'],
    ['Goggles of Night', 'magic-items/4648-goggles-of-night'],
    ['Gray Bag of Tricks', 'magic-items/4582-bag-of-tricks'],
    //['Guardian Emblem', 'magic-items/2412282-guardian-emblem'],
    ['Hat of Disguise', 'magic-items/4651-hat-of-disguise'],
    ['Headband of Intellect', 'magic-items/4652-headband-of-intellect'],
    ['Helm of Comprehending Languages', 'magic-items/4654-helm-of-comprehending-languages'],
    ['Helm of Telepathy', 'magic-items/4655-helm-of-telepathy'],
    ['Immovable Rod', 'magic-items/4662-immovable-rod'],
    //['Instrument of the Bards, Doss Lute', 'magic-items/5508-instrument-of-the-bards'],
    //['Instrument of the Bards, Fochlucan Bandore', 'magic-items/5508-instrument-of-the-bards'],
    //['Instrument of the Bards, Mac-Fuirmidh Cittern', 'magic-items/5508-instrument-of-the-bards'],
    ['Javelin of Lightning', 'magic-items/4667-javelin-of-lightning'],
    //['Keoghtom\'s Ointment', 'magic-items/5355-keoghtoms-ointment'],
    ['Lantern of Revealing', 'magic-items/4668-lantern-of-revealing'],
    //['Mariner\'s Armor', 'magic-items/5507-mariners-armor'],
    ['Medallion of Thoughts', 'magic-items/4679-medallion-of-thoughts'],
    ['Mithral Armor', 'magic-items/5381-mithral-armor'],
    //['Moon Sickle, +1', 'magic-items/2405401-moon-sickle'],
    ['Necklace of Adaptation', 'magic-items/4682-necklace-of-adaptation'],
    ['Pearl of Power', 'magic-items/4691-pearl-of-power'],
    ['Periapt of Health', 'magic-items/4692-periapt-of-health'],
    ['Periapt of Wound Closure', 'magic-items/4694-periapt-of-wound-closure'],
    ['Pipes of Haunting', 'magic-items/4696-pipes-of-haunting'],
    ['Pipes of the Sewers', 'magic-items/4697-pipes-of-the-sewers'],
    //['Quiver of Ehlonna', 'magic-items/5367-quiver-of-ehlonna'],
    //['Rhythm Maker\'s Drum, +1', 'magic-items/2405713-rhythm-makers-drum'],
    ['Ring of Jumping', 'magic-items/4724-ring-of-jumping'],
    ['Ring of Mind Shielding', 'magic-items/4725-ring-of-mind-shielding'],
    ['Ring of Swimming', 'magic-items/4732-ring-of-swimming'],
    ['Ring of Warmth', 'magic-items/4736-ring-of-warmth'],
    ['Ring of Water Walking', 'magic-items/4737-ring-of-water-walking'],
    ['Robe of Useful Items', 'magic-items/4743-robe-of-useful-items'],
    //['Rod of the Pact Keeper, +1', 'magic-items/11214-rod-of-the-pact-keeper'],
    ['Rope of Climbing', 'magic-items/4749-rope-of-climbing'],
    ['Rust Bag of Tricks', 'magic-items/4582-bag-of-tricks'],
    //['Saddle of the Cavalier', 'magic-items/5396-saddle-of-the-cavalier'],
    //['Sending Stones', 'magic-items/5402-sending-stones'],
    //['Sentinel Shield', 'magic-items/5403-sentinel-shield'],
    ['Shield, +1', 'magic-items/4753-shield-1'],
    ['Slippers of Spider Climbing', 'magic-items/4755-slippers-of-spider-climbing'],
    //['Staff of the Adder', 'magic-items/5406-staff-of-the-adder'],
    ['Staff of the Python', 'magic-items/4768-staff-of-the-python'],
    ['Stone of Good Luck (Luckstone)', 'magic-items/4773-stone-of-good-luck-luckstone'],
    ['Tan Bag of Tricks', 'magic-items/4582-bag-of-tricks'],
    ['Trident of Fish Command', 'magic-items/4784-trident-of-fish-command'],
    ['Wand of Magic Detection', 'magic-items/4793-wand-of-magic-detection'],
    ['Wand of Magic Missiles', 'magic-items/4794-wand-of-magic-missiles'],
    ['Wand of Secrets', 'magic-items/4797-wand-of-secrets'],
    ['Wand of the War Mage, +1', 'magic-items/34712-wand-of-the-war-mage'],
    ['Wand of Web', 'magic-items/4799-wand-of-web'],
    ['Weapon, +1', 'magic-items/5400-weapon-1'],
    //['Weapon of Warning', 'magic-items/5504-weapon-of-warning'],
    ['Wind Fan', 'magic-items/4803-wind-fan'],
    ['Winged Boots', 'magic-items/4804-winged-boots']
];

const rare = [
    //['Alchemical Compendium', 'magic-items/2400596-alchemical-compendium'],
    //['All-Purpose Tool, +2', 'magic-items/2400634-all-purpose-tool'],
    ['Ammunition, +2', 'magic-items/5410-ammunition-2']
    ['Amulet of Health', 'magic-items/4568-amulet-of-health'],
    //['Amulet of the Devout, +2', 'magic-items/2411830-amulet-of-the-devout'],
    //['Arcane Grimoire, +2', 'magic-items/2411893-arcane-grimoire'],
    ['Armor, +1', 'magic-items/5377-armor-1'],
    ['Armor of Acid Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Cold Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Fire Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Force Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Lightning Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Necrotic Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Poison Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Psychic Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Radiant Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Armor of Thunder Resistance', 'magic-items/5371-armor-of-resistance'],
    ['Arrow-Catching Shield', 'magic-items/4577-arrow-catching-shield'],
    //['Astral Shard', 'magic-items/2374941-astral-shard'],
    //['Astromancy Archive', 'magic-items/2400685-astromancy-archive'],
    //['Atlas of Endless Horizons', 'magic-items/2400706-atlas-of-endless-horizons'],
    ['Bag of Beans', 'magic-items/4579-bag-of-beans'],
    //['Bell Branch', 'magic-items/2401130-bell-branch'],
    ['Belt of Dwarvenkind', 'magic-items/4584-belt-of-dwarvenkind'],
    ['Belt of Hill Giant Strength', 'magic-items/5372-belt-of-giant-strength'],
    //['Bloodwell Vial, +2', 'magic-items/2412091-bloodwell-vial'],
    ['Boots of Levitation', 'magic-items/4588-boots-of-levitation'],
    ['Boots of Speed', 'magic-items/4589-boots-of-speed'],
    ['Bowl of Commanding Water Elementals', 'magic-items/4592-bowl-of-commanding-water-elementals'],
    ['Bracers of Defense', 'magic-items/4594-bracers-of-defense'],
    ['Brazier of Commanding Fire Elementals', 'magic-items/4595-brazier-of-commanding-fire-elementals'],
    ['Cape of the Mountebank', 'magic-items/4599-cape-of-the-mountebank'],
    ['Censer of Commanding Air Elementals', 'magic-items/4601-censer-of-controlling-air-elementals'],
    ['Cloak of Displacement', 'magic-items/4605-cloak-of-displacement'],
    ['Cloak of the Bat', 'magic-items/4608-cloak-of-the-bat'],
    ['Cube of Force', 'magic-items/4611-cube-of-force'],
    //['Daern\'s Instant Fortress', 'magic-items/4663-instant-fortress'],
    ['Dagger of Venom', 'magic-items/4613-dagger-of-venom'],
    //['Devotee\'s Censer', 'magic-items/2407478-devotees-censer'],
    ['Dimensional Shackles', 'magic-items/4620-dimensional-shackles'],
    ['Dragon Slayer', 'magic-items/5385-dragon-slayer'],
    //['Dragonguard', 'magic-items/12467-dragonguard'],
    //['Duplicitous Manuscript', 'magic-items/2412157-duplicitous-manuscript'],
    //['Elemental Essence Shard, Air', 'magic-items/2407551-elemental-essence-shard'],
    //['Elemental Essence Shard, Earth', 'magic-items/2407551-elemental-essence-shard'],
    //['Elemental Essence Shard, Fire', 'magic-items/2407551-elemental-essence-shard'],
    //['Elemental Essence Shard, Water', 'magic-items/2407551-elemental-essence-shard'],
    ['Elven Chain', 'magic-items/4631-elven-chain'],
    //['Far Realm Shard', 'magic-items/2412226-far-realm-shard'],
    ['Figurine of Wondrous Power, Bronze Griffon', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Figurine of Wondrous Power, Ebony Fly', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Figurine of Wondrous Power, Golden Lions', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Figurine of Wondrous Power, Ivory Goats', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Figurine of Wondrous Power, Marble Elephants', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Figurine of Wondrous Power, Onyx Dog', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Figurine of Wondrous Power, Serpentine Owl', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Flame Tongue', 'magic-items/5386-flame-tongue'],
    ['Folding Boat', 'magic-items/4639-folding-boat'],
    //['Fulminating Treatise', 'magic-items/2412256-fulminating-treatise'],
    ['Gem of Seeing', 'magic-items/4643-gem-of-seeing'],
    ['Giant Slayer', 'magic-items/5388-giant-slayer'],
    ['Glamoured Studded Leather', 'magic-items/4645-glamoured-studded-leather'],
    ['Handy Haversack', 'magic-items/4650-handy-haversack'],
    //['Heart Weaver\'s Primer', 'magic-items/2412288-heart-weavers-primer'],
    ['Helm of Teleportation', 'magic-items/4656-helm-of-teleportation'],
    //['Heward\'s Handy Haversack', 'magic-items/5353-hewards-handy-haversack'],
    ['Horn of Blasting', 'magic-items/4658-horn-of-blasting'],
    ['Horn of Valhalla, Brass', 'magic-items/5414-horn-of-valhalla'],
    ['Horn of Valhalla, Silver', 'magic-items/5414-horn-of-valhalla'],
    ['Horseshoes of Speed', 'magic-items/4661-horseshoes-of-speed'],
    ['Instant Fortress', 'magic-items/4663-instant-fortress'],
    //['Instrument of the Bards, Canaith Mandolin', 'magic-items/5508-instrument-of-the-bards'],
    //['Instrument of the Bards, Cli Lyre', 'magic-items/5508-instrument-of-the-bards'],
    ['Ioun Stone of Awareness', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Historical Knowledge', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Language Knowledge', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Natural Knowledge', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Protection', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Religious Knowledge', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Reserve', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Self Preservation', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Sustenance', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Supreme Intellect', 'magic-items/5415-ioun-stone'],
    ['Iron Bands of Bilarro', 'magic-items/5354-iron-bands-of-bilarro'],
    //['Libram of Souls and Flesh', 'magic-items/2412349-libram-of-souls-and-flesh'],
    //['Lyre of Building', 'magic-items/2405289-lyre-of-building'],
    ['Mace of Disruption', 'magic-items/4670-mace-of-disruption'],
    ['Mace of Smiting', 'magic-items/4671-mace-of-smiting'],
    ['Mace of Terror', 'magic-items/4672-mace-of-terror'],
    ['Mantle of Spell Resistance', 'magic-items/4673-mantle-of-spell-resistance'],
    ['Master\'s Amulet', 'magic-items/81901-masters-amulet'],
    //['Moon Sickle, +2', 'magic-items/2405401-moon-sickle'],
    ['Necklace of Fireballs', 'magic-items/4683-necklace-of-fireballs'],
    ['Necklace of Prayer Beads', 'magic-items/4684-necklace-of-prayer-beads'],
    //['Outer Essence Shard', 'magic-items/2405522-outer-essence-shard'],
    ['Periapt of Proof Against Poison', 'magic-items/4693-periapt-of-proof-against-poison'],
    //['Planecaller\'s Codex', 'magic-items/2405636-planecallers-codex'],
    ['Portable Hole', 'magic-items/4699-portable-hole'],
    //['Protective Verses', 'magic-items/2405662-protective-verses'],
    //['Reveler\'s Concertina', 'magic-items/2405691-revelers-concertina'],
    //['Rhythm Maker\'s Drum, +2', 'magic-items/2405713-rhythm-makers-drum'],
    ['Ring of Acid Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Animal Influence', 'magic-items/4717-ring-of-animal-influence'],
    ['Ring of Cold Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Evasion', 'magic-items/4720-ring-of-evasion'],
    ['Ring of Feather Falling', 'magic-items/4721-ring-of-feather-falling'],
    ['Ring of Fire Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Force Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Free Action', 'magic-items/4722-ring-of-free-action'],
    ['Ring of Lightning Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Necrotic Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Poison Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Protection', 'magic-items/4726-ring-of-protection'],
    ['Ring of Psychic Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Radiant Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of Spell Storing', 'magic-items/4730-ring-of-spell-storing'],
    ['Ring of the Ram', 'magic-items/4734-ring-of-the-ram'],
    ['Ring of Thunder Resistance', 'magic-items/5420-ring-of-resistance'],
    ['Ring of X-ray Vision', 'magic-items/4738-ring-of-x-ray-vision'],
    ['Robe of Eyes', 'magic-items/4739-robe-of-eyes'],
    ['Rod of Rulership', 'magic-items/4747-rod-of-rulership'],
    //['Rod of the Pact Keeper, +2', 'magic-items/11214-rod-of-the-pact-keeper'],
    ['Rope of Entanglement', 'magic-items/4750-rope-of-entanglement'],
    //['Scroll of Protection Against Aberrations', 'magic-items/5398-scroll-of-protection'],
    //['Scroll of Protection Against Beasts', 'magic-items/5398-scroll-of-protection'],
    //['Scroll of Protection Against Celestials', 'magic-items/5398-scroll-of-protection'],
    //['Scroll of Protection Against Elementals', 'magic-items/5398-scroll-of-protection'],
    //['Scroll of Protection Against Fey', 'magic-items/5398-scroll-of-protection'],
    //['Scroll of Protection Against Fiends', 'magic-items/5398-scroll-of-protection'],
    //['Scroll of Protection Against Plants', 'magic-items/5398-scroll-of-protection'],
    //['Scroll of Protection Against Undead', 'magic-items/5398-scroll-of-protection'],
    //['Shadowfell Shard', 'magic-items/2408242-shadowfell-shard'],
    ['Shield, +2', 'magic-items/5157-shield-2'],
    //['Spider Staff', 'magic-items/9088-spider-staff'],
    ['Staff of Charming', 'magic-items/4760-staff-of-charming'],
    ['Staff of Healing', 'magic-items/4763-staff-of-healing'],
    ['Staff of Swarming Insects', 'magic-items/4766-staff-of-swarming-insects'],
    ['Staff of the Woodlands', 'magic-items/4769-staff-of-the-woodlands'],
    ['Staff of Withering', 'magic-items/4771-staff-of-withering'],
    ['Stone of Controlling Earth Elementals', 'magic-items/4772-stone-of-controlling-earth-elementals'],
    ['Sun Blade', 'magic-items/4774-sun-blade'],
    ['Sword of Life Stealing', 'magic-items/5390-sword-of-life-stealing'],
    ['Sword of Wounding', 'magic-items/5392-sword-of-wounding'],
    //['Tentacle Rod', 'magic-items/5408-tentacle-rod'],
    ['Vicious Weapon', 'magic-items/5399-vicious-weapon'],
    ['Wand of Binding', 'magic-items/4788-wand-of-binding'],
    ['Wand of Enemy Detection', 'magic-items/4789-wand-of-enemy-detection'],
    ['Wand of Fear', 'magic-items/4790-wand-of-fear'],
    ['Wand of Fireballs', 'magic-items/4791-wand-of-fireballs'],
    ['Wand of Lightning Bolts', 'magic-items/4792-wand-of-lightning-bolts'],
    ['Wand of Paralysis', 'magic-items/4795-wand-of-paralysis'],
    ['Wand of the War Mage, +2', 'magic-items/34712-wand-of-the-war-mage'],
    ['Wand of Wonder', 'magic-items/4800-wand-of-wonder'],
    ['Weapon, +2', 'magic-items/5401-weapon-2'],
    ['Wings of Flying', 'magic-items/4805-wings-of-flying']
];

const veryRare = [
    //['All-Purpose Tool, +3', 'magic-items/2400634-all-purpose-tool'],
    ['Ammunition, +3', 'magic-items/5411-ammunition-3'],
    //['Amulet of the Devout, +3', 'magic-items/2411830-amulet-of-the-devout'],
    ['Amulet of the Planes', 'magic-items/4570-amulet-of-the-planes'],
    ['Animated Shield', 'magic-items/4571-animated-shield'],
    //['Arcane Grimoire, +3', 'magic-items/2411893-arcane-grimoire'],
    ['Armor, +2', 'magic-items/5378-armor-2'],
    ['Arrow of Slaying', 'magic-items/4578-arrow-of-slaying'],
    ['Bag of Devouring', 'magic-items/4580-bag-of-devouring'],
    ['Belt of Fire Giant Strength', 'magic-items/5372-belt-of-giant-strength'],
    ['Belt of Frost Giant Strength', 'magic-items/5372-belt-of-giant-strength'],
    ['Belt of Stone Giant Strength', 'magic-items/5372-belt-of-giant-strength'],
    //['Bloodwell Vial, +3', 'magic-items/2412091-bloodwell-vial'],
    ['Carpet of Flying', 'magic-items/5373-carpet-of-flying'],
    //['Cauldron of Rebirth', 'magic-items/2401168-cauldron-of-rebirth'],
    ['Cloak of Arachnida', 'magic-items/4604-cloak-of-arachnida'],
    ['Crystal Ball', 'magic-items/4610-crystal-ball'],
    //['Crystalline Chronicle', 'magic-items/2407470-crystalline-chronicle'],
    ['Dancing Sword', 'magic-items/5383-dancing-sword'],
    ['Dragon Scale Mail, Black', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, Blue', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, Brass', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, Bronze', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, Copper', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, Gold', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, Green', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, Red', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, Silver', 'magic-items/5380-dragon-scale-mail'],
    ['Dragon Scale Mail, White', 'magic-items/5380-dragon-scale-mail'],
    ['Dwarven Plate', 'magic-items/4626-dwarven-plate'],
    ['Dwarven Thrower', 'magic-items/4627-dwarven-thrower'],
    ['Figurine of Wondrous Power, Obsidian Steed', 'magic-items/5413-figurine-of-wondrous-power'],
    ['Frost Brand', 'magic-items/5387-frost-brand'],
    ['Helm of Brilliance', 'magic-items/4653-helm-of-brilliance'],
    ['Horn of Valhalla, Bronze', 'magic-items/5414-horn-of-valhalla'],
    ['Horn of Valhalla, Bronze', 'magic-items/5414-horn-of-valhalla'],
    ['Horseshoes of a Zephyr', 'magic-items/4660-horseshoes-of-a-zephyr'],
    //['Instrument of the Bards, Anstruth Harp', 'magic-items/5508-instrument-of-the-bards'],
    ['Ioun Stone of Absorption', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Agilty', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Fortitude', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Insight', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Intellect', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Leadership', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Strength', 'magic-items/5415-ioun-stone'],
    ['Manual of Bodily Health', 'magic-items/4674-manual-of-bodily-health'],
    ['Manual of Gainful Exercise', 'magic-items/4675-manual-of-gainful-exercise'],
    ['Manual of Quickness of Action', 'magic-items/4677-manual-of-quickness-of-action'],
    //['Moon Sickle, +3', 'magic-items/2405401-moon-sickle'],
    ['Mirror of Life Trapping', 'magic-items/4680-mirror-of-life-trapping'],
    ['Nine Lives Stealer', 'magic-items/5394-nine-lives-stealer'],
    ['Oathbow', 'magic-items/4686-oathbow'],
    //['Rhythm Maker\'s Drum, +3', 'magic-items/2405713-rhythm-makers-drum'],
    ['Ring of Regeneration', 'magic-items/4727-ring-of-regeneration'],
    ['Ring of Shooting Stars', 'magic-items/4729-ring-of-shooting-stars'],
    ['Ring of Telekinesis', 'magic-items/4733-ring-of-telekinesis'],
    ['Robe of Scintillating Colors', 'magic-items/4740-robe-of-scintillating-colors'],
    ['Robe of Stars', 'magic-items/4741-robe-of-stars'],
    ['Rod of Absorption', 'magic-items/4744-rod-of-absorption'],
    ['Rod of Alertness', 'magic-items/4745-rod-of-alertness'],
    ['Rod of Security', 'magic-items/4748-rod-of-security'],
    //['Rod of the Pact Keeper, +1', 'magic-items/11214-rod-of-the-pact-keeper'],
    ['Scimitar of Speed', 'magic-items/4752-scimitar-of-speed'],
    ['Shield, +3', 'magic-items/5158-shield-3'],
    ['Spellguard Shield', 'magic-items/4757-spellguard-shield'],
    ['Staff of Fire', 'magic-items/4761-staff-of-fire'],
    ['Staff of Frost', 'magic-items/4762-staff-of-frost'],
    ['Staff of Power', 'magic-items/4764-staff-of-power'],
    ['Staff of Striking', 'magic-items/4765-staff-of-striking'],
    ['Staff of Thunder and Lightning', 'magic-items/4770-staff-of-thunder-and-lightning'],
    ['Sword of Sharpness', 'magic-items/5391-sword-of-sharpness'],
    ['Tome of Clear Thought', 'magic-items/4781-tome-of-clear-thought'],
    ['Tome of Leadership and Influence', 'magic-items/4782-tome-of-leadership-and-influence'],
    ['Tome of Understanding', 'magic-items/4783-tome-of-understanding'],
    ['Wand of Polymorph', 'magic-items/4796-wand-of-polymorph'],
    ['Wand of the War Mage, +3', 'magic-items/34712-wand-of-the-war-mage'],
    ['Weapon, +3', 'magic-items/5404-weapon-3']
];

const legendary = [
    //['Apparatus of Kwalish', 'magic-items/5344-apparatus-of-kwalish'],
    ['Apparatus of the Crab', 'magic-items/4572-apparatus-of-the-crab'],
    ['Armor, +3', 'magic-items/5379-armor-3'],
    ['Armor of Invulnerability', 'magic-items/4574-armor-of-invulnerability'],
    ['Belt of Cloud Giant Strength', 'magic-items/5372-belt-of-giant-strength'],
    ['Belt of Storm Giant Strength', 'magic-items/5372-belt-of-giant-strength'],
    //['Blackrazor', 'magic-items/5429-blackrazor'],
    //['Cloak of Invisibility', 'magic-items/5347-cloak-of-invisibility'],
    ['Crystal Ball of Mind Reading', 'magic-items/4610-crystal-ball'],
    ['Crystal Ball of Telepathy', 'magic-items/4610-crystal-ball'],
    ['Crystal Ball of True Seeing', 'magic-items/4610-crystal-ball'],
    ['Cubic Gate', 'magic-items/4612-cubic-gate'],
    ['Defender', 'magic-items/5384-defender'],
    //['Efreeti Chain', 'magic-items/5350-efreeti-chain'],
    ['Hammer of Thunderbolts', 'magic-items/4649-hammer-of-thunderbolts'],
    ['Holy Avenger', 'magic-items/5389-holy-avenger'],
    ['Horn of Valhalla, Iron', 'magic-items/5414-horn-of-valhalla'],
    //['Instrument of the Bards, Ollamh Harp', 'magic-items/5508-instrument-of-the-bards'],
    ['Ioun Stone of Greater Absorption', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Mastery', 'magic-items/5415-ioun-stone'],
    ['Ioun Stone of Regeneration', 'magic-items/5415-ioun-stone'],
    ['Iron Flask', 'magic-items/4666-iron-flask'],
    ['Luck Blade', 'magic-items/5393-luck-blade'],
    ['Plate Armor of Etherealness', 'magic-items/4698-plate-armor-of-etherealness'],
    ['Ring of Djinni Summoning', 'magic-items/4718-ring-of-djinni-summoning'],
    ['Ring of Elemental Command, Air', 'magic-items/5509-ring-of-elemental-command'],
    ['Ring of Elemental Command, Earth', 'magic-items/5509-ring-of-elemental-command'],
    ['Ring of Elemental Command, Fire', 'magic-items/5509-ring-of-elemental-command'],
    ['Ring of Elemental Command, Water', 'magic-items/5509-ring-of-elemental-command'],
    ['Ring of Invisibility', 'magic-items/4723-ring-of-invisibility'],
    ['Ring of Spell Turning', 'magic-items/4731-ring-of-spell-turning'],
    ['Robe of the Archmagi', 'magic-items/4742-robe-of-the-archmagi'],
    ['Rod of Lordly Might', 'magic-items/4746-rod-of-lordly-might'],
    //['Rod of Resurrection', 'magic-items/5395-rod-of-resurrection'],
    ['Sphere of Annihilation', 'magic-items/4759-sphere-of-annihilation'],
    ['Staff of the Magi', 'magic-items/4767-staff-of-the-magi'],
    //['Sword of Answering', 'magic-items/5506-sword-of-answering'],
    ['Talisman of Pure Good', 'magic-items/4778-talisman-of-pure-good'],
    ['Talisman of the Sphere', 'magic-items/4779-talisman-of-the-sphere'],
    ['Talisman of Ultimate Evil', 'magic-items/4780-talisman-of-ultimate-evil'],
    //['Tome of the Stilled Tongue', 'magic-items/5409-tome-of-the-stilled-tongue'],
    ['Vorpal Sword', 'magic-items/5397-vorpal-sword'],
    //['Wave', 'magic-items/5431-wave'],
    ['Well of Many Worlds', 'magic-items/4802-well-of-many-worlds'],
    //['Whelm', 'magic-items/5432-whelm'],
    ['Orb of Dragonkind', 'magic-items/4690-orb-of-dragonkind']
];